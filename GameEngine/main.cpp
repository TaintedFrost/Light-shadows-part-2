...
//s3
//glm::vec3 memoryTilePos = glm::vec3(20, 0, 10);
//float memoryTileRadius = 4.0f;
glm::vec3 simonTilePos = glm::vec3(0.0f, 0.5f, 40.0f);
float simonTileRadius = 3.0f;
bool simonTriggered = false;

struct SimonVisual
{
	SimonShape shape;
	glm::vec3 offset;   // relative to camera
	glm::vec3 color;
};

std::vector<SimonVisual> simonVisuals =
{
	{ SimonShape::BLUE,   glm::vec3(-2.0f,  1.5f, -6.0f), glm::vec3(0.2f, 0.2f, 1.0f) },
	{ SimonShape::RED,    glm::vec3(2.0f,  1.5f, -6.0f), glm::vec3(1.0f, 0.2f, 0.2f) },
	{ SimonShape::GREEN,  glm::vec3(-2.0f, -1.0f, -6.0f), glm::vec3(0.2f, 1.0f, 0.2f) },
	{ SimonShape::YELLOW, glm::vec3(2.0f, -1.0f, -6.0f), glm::vec3(1.0f, 1.0f, 0.2f) }
};
...
int main()
{
		...
		glfwWindowShouldClose(window.getWindow()) == 0)
	{
		...
		/*if (!task2Completed && gameState == GameState::WORLD)
		{
			if (distance(camera.getCameraPosition(), memoryTilePos) < memoryTileRadius)
			{
				startSimonGame();
			}
		}
		...
		if (window.isMousePressed(GLFW_MOUSE_BUTTON_LEFT))
		{
			...
			handleSimonClick(mx, my);
		}*/
		if (!task2Completed && !simonTriggered && gameState == GameState::WORLD)
		{
			float dist = glm::distance(
				glm::vec3(camera.getCameraPosition().x, 0.0f, camera.getCameraPosition().z),
				glm::vec3(simonTilePos.x, 0.0f, simonTilePos.z)
			);

			if (dist < simonTileRadius)
			{
				simonTriggered = true;
				startSimonGame();
			}
		...
		{
			glm::mat4 model = glm::mat4(1.0f);
			model = glm::translate(model, simonTilePos);
			model = glm::scale(model, glm::vec3(6.0f, 0.3f, 6.0f));
